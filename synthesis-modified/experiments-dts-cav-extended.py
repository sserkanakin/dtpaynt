...
def run_paynt(paynt_dir, sketch, options, timeout_seconds, maxmem_mb, task_name, output_dir, restart:bool, q3:bool, epsilon_optimal_stop:float=None):
    ...
    # Add epsilon-optimal-stop if provided
    if epsilon_optimal_stop is not None:
        options += f" --epsilon-optimal-stop {epsilon_optimal_stop}"
...
def evaluate_benchmarks(experiment_name, num_workers, paynt_dir, timeout_seconds, maxmem_mb, options, restart, output_dir, models_dir, q3, epsilon_optimal_stop:float=None):
    ...
    if num_workers == 1:
        for sketch,task_name,task_output_dir in tasks:
            run_paynt(paynt_dir, sketch, options, timeout_seconds, maxmem_mb, task_name, task_output_dir, restart, q3, epsilon_optimal_stop)
    else:
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
            for (sketch,task_name,task_output_dir) in tasks:
                executor.submit(run_paynt, paynt_dir, sketch, options, timeout_seconds, maxmem_mb, task_name, task_output_dir, restart, q3, epsilon_optimal_stop)
...
def create_table_row(model, path, latex=False):
    ...
    r.relevant_states = float_pretty(search_int(lines, r"MDP has (\d+)/.*? relevant states") / r.states * 100) if r.states else None
...
@click.option('--experiment-name', type=str, help='Name of the experiments.')
...
@click.option('--show-only', is_flag=True, default=False, help='Show results only.')
@click.option('--generate-csv', is_flag=True, default=False, help='Generate CSV file with results.')
@click.option('--smoke-test', is_flag=True, default=False, help='Smoke test setting.')
@click.option('--restart', is_flag=True, default=False, help='Re-run all benchmarks.')
@click.option('--q3', is_flag=True, default=False, help='Use Q3 settings.')
@click.option('--epsilon-optimal-stop', type=click.FLOAT, default=None, help='Stop synthesis early if a solution within this absolute epsilon of the optimum is found.')
def main(paynt_dir, models_dir, workers, timeout, maxmem, output, experiment_name, depth_min, depth_max, show_only, generate_csv, smoke_test, restart, q3, epsilon_optimal_stop):
    print(f"Running experiments with epsilon_optimal_stop={epsilon_optimal_stop}")
    
    if experiment_name is None:
        raise click.ClickException("--experiment-name is required")

    maxmem_mb = maxmem*1024

    if show_only:
        show_experiment(experiment_name, output, spreadsheet=generate_csv)
        return

    print(f"----- experiment: {experiment_name} -----")
    evaluate_benchmarks(experiment_name, workers, paynt_dir, timeout, maxmem_mb, "", restart, output, models_dir, q3, epsilon_optimal_stop)
