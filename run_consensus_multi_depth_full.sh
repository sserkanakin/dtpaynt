#!/bin/bash
# Full multi-depth runner: depths 4, 5, 6, 7 with ALL alpha values
# Variants: Original + ValueOnly + ValueSize(0.01, 0.1, 0.25, 0.5, 0.75, 0.9, 0.99) + BoundsGap
# Usage: bash run_consensus_multi_depth_full.sh
...
echo "========================================================================"
echo "Consensus-4-2: Multi-Depth Full Comparison (Depths 4-7, All Alphas)"
echo "========================================================================"
...
for alpha in "${ALPHAS[@]}"; do
  run_variant_bg "D4-MOD-VALUE_SIZE-${alpha}"    "dtpaynt-modified"  "synthesis-modified"  "1800"  "4"  "value_size"  "${alpha}"
done
...
for alpha in "${ALPHAS[@]}"; do
  run_variant_bg "D5-MOD-VALUE_SIZE-${alpha}"    "dtpaynt-modified"  "synthesis-modified"  "3600"  "5"  "value_size"  "${alpha}"
done
...
for alpha in "${ALPHAS[@]}"; do
  run_variant_bg "D6-MOD-VALUE_SIZE-${alpha}"    "dtpaynt-modified"  "synthesis-modified"  "3600"  "6"  "value_size"  "${alpha}"
done
...
for alpha in "${ALPHAS[@]}"; do
  run_variant_bg "D7-MOD-VALUE_SIZE-${alpha}"    "dtpaynt-modified"  "synthesis-modified"  "3600"  "7"  "value_size"  "${alpha}"
done
run_variant_bg "D7-MOD-BOUNDS_GAP"        "dtpaynt-modified"  "synthesis-modified"  "3600"  "7"  "bounds_gap"  ""
echo ""

echo "[Parallel] All variants launched in background!"
echo "[Parallel] Total: 4 depths × 10 variants = 40 runs"
echo ""
echo "To monitor progress, use:"
echo "  tail -f /tmp/multi_depth_full_run.log"
...
for algo_dir in results_dir.iterdir():
    ...
    for bench_dir in algo_dir.iterdir():
        ...
        for run_dir in bench_dir.iterdir():
            ...
            # Extract depth from run-info.json
